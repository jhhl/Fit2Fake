...
class CorpusManager: NSObject
{
    ...
    public var sentences: [String]? // now just an array of strings
    ...
    func getSectionMap()
    {
        ...
        do{
            self.sectionMap = try JSONSerialization.jsonObject(with: rawJSONData!) as? Dictionary<String,String> ??
                [
                    // these are sections that are built into the php and need no apikey
                    "Info":"info",
                    "Privacy":"privacy",
                    "Tweets":"d",
                    "Declaration":"dofi",
                    "Bill Of Rights":"billor"
                ]
        }
        catch
        {
    ...
    /// fills and returns vocabulary. 
    ///
    /// - Parameter section: which section from the API
    func getJSON(section:String) -> [String]?
    {
        ...
        {
            print(error)
            sentences = fillWithTweets(reason: error.localizedDescription);
            return sentences
        }
        ...
        do
        {
            // should be an array of Strings, really
                let thisBatch:[String] = try (JSONSerialization.jsonObject(with: rawJSONData!) as? [String] ?? [])
            // it's actually the real vocs and rings
//            if vocabulary == nil
//            {
//                vocabulary = [String]()
//            }
//            vocabulary?.append(contentsOf: thisBatch)
            sentences = thisBatch
            ...
            if (sentences == nil)
            {
                ...
                sentences = fillWithTweets(reason:reason);
            }
        }
        catch
        {
    ...
    {
        guard let results:[String] = sentences
            else
        {
            return nil
        }
